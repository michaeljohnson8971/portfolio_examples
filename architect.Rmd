...
This example uses the `architect` theme, which is available if you install the `prettydoc` package. The `prettydoc` themes don't have the option to add a table of contents (as far as I can tell). 

Other themes available from the `prettydoc` package are: `cayman`, `tactile`, `leonids`, and `hpstr`.
